...
const round = (theirMove: Hand, myMove: Hand) =>
	getScore(theirMove, myMove) + myMove + 1;
...
const part1$ = play(([theirMove, myMove]) => {
	const assumedStrategy: Record<string, Hand> = {
		X: Hand.Rock,
		Y: Hand.Paper,
		Z: Hand.Scissors,
	};
...
const part2$ = play((input) => {
	const knownStrategy: Record<string, number> = {
		X: Scores.Loss,
		Y: Scores.Draw,
		Z: Scores.Win,
	};
